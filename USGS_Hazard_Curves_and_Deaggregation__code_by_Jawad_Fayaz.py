...
for ii in range(0,len(df)):   
    ...
    for j in range(0,len(imt_list)):
        ...
        try:
            ...
            try:
                ...
                response_1,response_2 = url_resp_values_haz(params_2[0],url_hazard_2)
                if j == 0:
                    DF_HAZARD_CURVES = pd.DataFrame.from_dict(response_1)
                    DF_HAZARD_CURVES = DF_HAZARD_CURVES.rename(columns={"xvalues": "Acceleration (g)"})
                DF_HAZARD_CURVES = pd.concat([DF_HAZARD_CURVES, pd.DataFrame.from_dict(response_2)], axis=1) 
                DF_HAZARD_CURVES = DF_HAZARD_CURVES.rename(columns={"yvalues": DF_Cols[j]})
            except:
                count = count + 1
                print('\t\t\t\t\t\t\t\t\t\t\t\t\tNo Response from USGS for '+DF_Cols[j])
                if count == len(imt_list):
                    raise ValueError('USGS Server Busy! No Response from USGS. Please try again after sometime.') 
    ...
    ### ---------- DEAGGREGATION ---------- ###   
    print('\n\nDownloading Deaggregation Results for Site Input {}...\n\n'.format(np.round(ii+1,0)))       
    lm=df[ii:ii+1].reset_index(drop=True)
    try:
        lm['vs30']    = np.int(lm['vs30'])
        params        = lm.apply(lambda x: sfmt(**x), 1)
        ...
        if data['status']=='busy':
            params        = lm.apply(lambda x: sfmt_2(**x), 1)
            params_deag   = lm.apply(lambda x: sfmt_2(**x), 1)
        ...
        try:
            ...
            #print('\t\t\t\t\t\t\t\t\t\t\t\t\tNo Response from USGS for Deaggregation')
            raise ValueError('USGS Server Busy! No Response from USGS. Please try again after sometime.')
